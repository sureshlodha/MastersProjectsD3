...
var margin_l_2_2 = {top_l_2_2: 20, right_l_2_2: 280, bottom_l_2_2: 30, left_l_2_2: 60},
...
var color_l_2_2 = d3.scale.ordinal()
        .range(["#fc4e2a", "#fd8d3c", "#feb24c",	"#fed976", "#ffeda0", "#ffffcc", "#980043",	"#dd1c77", "#df65b0","#c994c7", "#d4b9da"]);
...
d3.csv("data/fixed_year_data_output_3_reverse_line_1_2.csv", function(error, data){
    ...
    d3.max(countries, function(c) { return d3.max(c.values, function(v) { return v.killed - 950; }); })
    ]);
    ...
    var path = country.append("path")
      ...
      svg_l_2_2.selectAll("text.text_terror" + d.name.substring(d.name.length - 4, d.name.length)).style("font-size", "13px");})
      
      .on("mouseout", function(d) {
          d3.select(this).style("stroke-width", "1.5px");
          svg_l_2_2.selectAll("text.text_terror" + d.name.substring(d.name.length - 4, d.name.length)).style("font-size", "10px");
...
//          }
    ...
    country.append("text")
      ...
      .attr("transform", function(d) { 
        if (d.name === "Al-Qaida in the Arabian Peninsula"|| d.name === "Al-Qaida in Iraq") {
            return "translate(" + x_l_2_2(d.value.year) + "," + y_l_2_2(d.value.killed - 35) + ")";
            
        } else if (d.name === "Communist Party of India - Maoist" || d.name === "Revolutionary Armed Forces of Colombia") {
            return "translate(" + x_l_2_2(d.value.year) + "," + y_l_2_2(d.value.killed + 40) + ")";
        } else if (d.name === "Al-Qaida") {
            return "translate(" + x_l_2_2(d.value.year) + "," + y_l_2_2(d.value.killed - 75) + ")";
        } else if (d.name === "Lord's Resistance Army") {
            return "translate(" + x_l_2_2(d.value.year) + "," + y_l_2_2(d.value.killed + 13) + ")";
        } else if (d.name === "Liberation Tigers of Tamil Eelam") {
            return "translate(" + x_l_2_2(d.value.year) + "," + y_l_2_2(d.value.killed + 5) + ")";
        }
        
        else {
            return "translate(" + x_l_2_2(d.value.year) + "," + y_l_2_2(d.value.killed) + ")";
        }
        })
